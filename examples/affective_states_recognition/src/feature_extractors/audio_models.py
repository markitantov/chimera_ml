...
class W2V2(Wav2Vec2PreTrainedModel):
    def __init__(self, config: Any) -> None:
...
class ExHuBERTEncoder(nn.Module):
    def __init__(self, config: Any) -> None:
        ...
        self.layer_norm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout)
        self.layers = nn.ModuleList([ExHuBERTEncoderLayer(config) for _ in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        for layer in self.layers:
            ...
            if not skip_the_layer or deepspeed_zero3_is_enabled:
                ...
                # Not sure if it's worth the added code complexity.
...
class ExHuBERT(HubertPreTrainedModel, PyTorchModelHubMixin):
    ...
    def freeze_og_encoder(self) -> None:
